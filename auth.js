...
async function loginOrSignup(mode) {
  ...
  if (mode === 'signup' && !data.session) {
    ...
    errEl.textContent = 'Compte créé. Confirmer l\'email reçu, puis cliquer de nouveau sur « Se connecter ».';
    return;
  }
...
async function saveProfile() {
  ...
  errEl.textContent = '';
  if (!doc) { errEl.textContent = 'Nom de médecin à choisir'; return; }
...
async function sendPasswordReset() {
  ...
  if (!email) {
    errEl.textContent = 'Email à saisir ci-dessus, puis cliquer de nouveau sur « Mot de passe oublié ? ».';
    return;
  }
  ...
  errEl.style.color = '#16a34a';
  errEl.textContent = 'Email envoyé ! Cliquer sur le lien reçu pour choisir un nouveau mot de passe.';
}
